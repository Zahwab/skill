...
echo "${CYAN_TEXT}${BOLD_TEXT}        Welcome to Tutorials         ${RESET_FORMAT}"
echo "${CYAN_TEXT}${BOLD_TEXT}╚════════════════════════════════════════════════════════╝${RESET_FORMAT}"
echo
...
for cluster in "${CLUSTERS[@]}"; do
  ...
  while [ "$status" != "RUNNING" ]; do
    ...
    echo "${MAGENTA_TEXT}💡 Tip: While waiting, you can check out my channel for more awesome tutorials! ${RESET_FORMAT}"
...
while true; do
  ...
  echo "${YELLOW_TEXT}${BOLD_TEXT}   Current rollout status: ${WHITE_TEXT}${BOLD_TEXT}$status${RESET_FORMAT}${YELLOW_TEXT}. Waiting... (Check my channel for tips!)${RESET_FORMAT}"
  for i in $(seq 10 -1 1); do
...
while true; do
  ...
  echo "${YELLOW_TEXT}${BOLD_TEXT}   Current rollout status: ${WHITE_TEXT}${BOLD_TEXT}$status${RESET_FORMAT}${YELLOW_TEXT}. Waiting... (My channel has great content! )${RESET_FORMAT}"
  for i in $(seq 10 -1 1); do
    echo -ne "${YELLOW_TEXT}${BOLD_TEXT}   Checking again in $i seconds... \r${RESET_FORMAT}"
    sleep 1
  done
...
while true; do
  ...
  echo "${YELLOW_TEXT}${BOLD_TEXT}   Current rollout status: ${WHITE_TEXT}${BOLD_TEXT}$status${RESET_FORMAT}${YELLOW_TEXT}. Waiting... (Find more at my channel!)${RESET_FORMAT}"
  for i in $(seq 10 -1 1); do
    echo -ne "${YELLOW_TEXT}${BOLD_TEXT}   Checking again in $i seconds... \r${RESET_FORMAT}"
    sleep 1
  done
...
echo "${BLUE_TEXT}${BOLD_TEXT}${UNDERLINE_TEXT}{RESET_FORMAT}"
echo